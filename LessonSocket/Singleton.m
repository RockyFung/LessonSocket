...
// socket连接
-(BOOL)socketConnectHostWithHost:(NSString *)host port:(NSInteger)port delegate:(id)delegate{
    self.socket    = [[GCDAsyncSocket alloc]initWithDelegate:delegate delegateQueue:dispatch_get_main_queue()];
    
    NSError *error = nil;
   
   BOOL result = [self.socket connectToHost:host onPort:port withTimeout:3 error:&error];
   return result;
}
// 连接成功回调
#pragma mark  - 连接成功回调
...
-(void)cutOffSocket{
    ...
    self.socket.userData = @"用户断开";// SocketOfflineByUser;
